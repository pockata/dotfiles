...
export FZF_DEFAULT_OPTS='--multi --bind=ctrl-k:down,ctrl-l:up --bind=alt-p:toggle-preview'
...
if command -v fdfind > /dev/null; then
	...
	export FZF_ALT_C_COMMAND="fd --follow --strip-cwd-prefix --type d 2> /dev/null"
fi
...
gb() {
    is_in_git_repo || return
    git branch -a --color=always | grep -v '/HEAD\s' | sed 's#remotes/##' |
    fzf-tmux --ansi --multi --preview-window right:60%:noborder \
